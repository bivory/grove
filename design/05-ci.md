...
| Platform | Target Triple | Release | Binary Name |
|----------|---------------|---------|-------------|
| Linux x86_64 | x86_64-unknown-linux-gnu | Yes | grove-x86_64-unknown-linux-gnu |
| Linux ARM64 | aarch64-unknown-linux-gnu | Yes | grove-aarch64-unknown-linux-gnu |
| macOS ARM64 | aarch64-apple-darwin | Yes | grove-aarch64-apple-darwin |
| macOS x86_64 | x86_64-apple-darwin | No | - |
| Windows | - | No | - |
...
The install script (`.claude-plugin/install.sh`) detects the platform and downloads
the appropriate binary using target triples:
...
**Note:** macOS x86_64 (Intel Macs) is not supported. Apple Silicon (ARM64) is required.
